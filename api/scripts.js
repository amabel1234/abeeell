...
function publicProduct(item) {
  ...
  return {
    ...item,
    scriptUrl: hasScript ? `/api/scripts-file?id=${encodeURIComponent(item.id)}&type=script` : "",
    imageUrl: hasImage ? `/api/scripts-file?id=${encodeURIComponent(item.id)}&type=image` : "",
  };
}
...
export default async function handler(req, res) {
  try {
    ...
    if (imageBase64) {
      ...
      imageUrl = `https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/${imagePath}`;
    }
